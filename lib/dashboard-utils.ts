 ...
 *
 * Rows are sorted by this resolved event time (desc) *before* the limit
 * is applied, so a session started long ago but completed recently
 * correctly appears at the top of the feed rather than being dropped.
 */
export function mapActivityItems(
  ...
  return sessions
    ...
    // Sort by resolved event time descending so recently-completed sessions
    // surface at the top even if their created_at is old.
    .sort(
      (a, b) =>
